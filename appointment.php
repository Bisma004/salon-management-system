...
if (strlen($_SESSION['loggedin'] == false)) {
	...
	if (isset($_POST['submit'])) {
		...
		$query = mysqli_query($con, "insert into  tblcustomers(AptNumber,Name,Email,MobileNumber,Gender,Details,app_date,app_time) value('$aptnumber','$name','$email','$mobilenum','$gender','$details','$date','$time' '$staff')");
		if ($query) {
    ...
    <textarea type="text" class="form-control" id="staff" name="staff"
        placeholder="Enter the staff id from whom  you want service" required="true" rows="2" cols="4"></textarea>
</div>
